...
function enterNum() {
  var user_in = confirm("Would you like to enter a number?");

  if(user_in){
      user_in = prompt("Enter the number.");
      if(isNaN(user_in)){
        alert("You did not enter a number!");
      }
      else {
        user_in = parseFloat(user_in);
        var result_message = "";

        if(isEven(user_in)){
          result_message = "The number is even\n";
        } else {
          result_message = "The number is odd\n";
        }

        result_message += ("The number plus 100 is " + plus100(user_in) + "\n");

        if(isPositive(user_in)){
          result_message += "The number is positive.";
        } else {
          result_message += "The number is negative.";
        }

        alert(result_message);
      }
  } else {
    console.log("The user doesn't want to participate. :(");
  }
}
